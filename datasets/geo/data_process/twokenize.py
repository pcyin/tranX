...
Contractions = re.compile(
    u"(?i)(\w+)(n['’′]t|['’′]ve|['’′]ll|['’′]d|['’′]re|['’′]s|['’′]m)$", re.UNICODE)
...
sadMouths = r"[\(\[\{]+"
tongue = "[pPd3]+"
# remove forward slash if http://'s aren't cleaned
...
basicface = "(?:" + bfLeft + bfCenter + bfRight + ")|" + \
    s3 + "|" + s4 + "|" + s5
...
emoticon = regex_or(
    ...
    "(?:>|&gt;)?" + regex_or(normalEyes, wink) + regex_or(noseArea, "[Oo]") + regex_or(
        tongue + r"(?=\W|$|RT|rt|Rt)", otherMouths + r"(?=\W|$|RT|rt|Rt)", sadMouths, happyMouths),

    # reversed version (: D:  use positive lookbehind to remove "(word):"
    # because eyes on the right side is more ambiguous with the standard
    # usage of : ;
    regex_or("(?<=(?: ))", "(?<=(?:^))") + regex_or(sadMouths, happyMouths,
                                                    otherMouths) + noseArea + regex_or(normalEyes, wink) + "(?:<|&lt;)?",
...
re_url = re.compile(unicode(url.decode('utf-8')), re.UNICODE)


def replaceUrlWithDomain(s):
  rp_list = []
  for m in re_url.finditer(s):
    st, end = m.span()
    url_find = s[st:end]
    domain = urlparse.urlparse(url_find).netloc
    rp_list.append((url_find, domain))
  for u, d in rp_list:
    s = s.replace(u, '_url_%s' % (d,))
  return s
